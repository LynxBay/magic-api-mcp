...
export const listApisTool: ToolDef<{ group?: string }, any[]> = {
  ...
  handler: async (client, args) => {
    ...
    const groupNames = collectGroupNameMap(tree);
    return files
...
/** 遍历整树，收集 groupId → 分组名 */
function collectGroupNameMap(tree: any): Map<string, string> {
  const map = new Map<string, string>();
  for (const folder of Object.keys(tree)) {
    const walk = (node: any): void => {
      for (const c of node.children ?? []) {
        if (c.node.method === undefined) map.set(c.node.id, c.node.name);
        walk(c);
      }
    };
    if (tree[folder]) walk(tree[folder]);
  }
  return map;
}
